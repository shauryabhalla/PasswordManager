...
- **Key Libraries:**
  ...
  - `pynput.keyboard` — simulate keystrokes to type passwords
  - `pyperclip` — copy-to-clipboard functionality

- **Install libraries (run in terminal, pip required):** `pip install pynput pystray Pillow cryptography pyperclip`
- **Check if pip is installed (run in terminal):** `pip --version`
...
- **Line 185 main.py to change it to any other key**
...
1. Install Python 3.x from python.org or raise Non catalog software request from 1C (check "Add to PATH").
2. `pip install pynput pystray Pillow cryptography pyperclip`
...
5. Daily use: `pythonw.exe main.py` (no console window) or create a shortcut. 
    Command needs to be changed according to where file is saved, for my case 
